...
let Documents = class Documents {
    ...
    get validDocumentCount() {
        return this.documents
            .filter(document => document.complianceStatus ===
            'VALID')
            .length;
    }
    /* =======================================================
    ...
    submit() {
        ...
        this.successMessage =
            '';
        if (!this.formValidationService.validateAndScroll(this.documentForm)) {
            return;
        }
        ...
        const values = this.documentForm
            .getRawValue();
        /*
        ...
        */
        if (values.issueDate &&
            values.expiryDate) {
            const issueDate = new Date(`${values.issueDate}T00:00:00`);
    ...
    renewDocument(document) {
        ...
        const fileInput = window.document.getElementById('fileUpload');
        if (fileInput) {
            fileInput.value = '';
        }
