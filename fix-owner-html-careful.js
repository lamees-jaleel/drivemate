...
html = html.replace(
    /<button class="stat-card" type="button">\s*<div class="stat-card-top">\s*<span class="stat-icon">\s*!\s*<\/span>\s*<span class="stat-label">\s*COMPLIANCE ALERTS\s*<\/span>/,
    '<button class="stat-card" type="button" (click)="openModule(\'Documents & Compliance\')">\n          <div class="stat-card-top">\n            <span class="stat-icon"> ! </span>\n            <span class="stat-label"> COMPLIANCE ALERTS </span>'
);
...
html = html.replace(/Manage Documents/g, 'Manage Documents & Compliance');

// 4. Update Feature Grid Empty States
// Maintenance
...
// Documents
html = html.replace(/<div class="document-status-list">[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>/, 
    `<div class="document-status-list">
            <div style="width: 100%; display: flex; justify-content: space-between;">
              <span> Status </span>
              @if (complianceAlerts > 0) {
                <span class="status-empty" style="color: #ff3545; background: rgba(255,53,69,0.1)"> {{ complianceAlerts }} alerts </span>
              } @else {
                <span class="status-empty" style="color: #28a745; background: rgba(40,167,69,0.1)"> Compliant </span>
              }
            </div>
          </div>`);
...
console.log("Careful HTML fix done");
